...
bool GSDevice11::Create(const std::shared_ptr<GSWnd>& wnd)
{
	...
	m_factory = D3D::CreateFactory(enable_debugging);
	if (!m_factory)
		return false;

	// select adapter
	CComPtr<IDXGIAdapter1> adapter = D3D::GetAdapterFromIndex(
		m_factory, theApp.GetConfigI("adapter_index")
	);
	...
	// device creation
	{
		...
		HRESULT result = D3D11CreateDevice(
			...
			D3D11_SDK_VERSION, &m_dev, &feature_level, &m_ctx
		);
...

	{
		...
		m_hack_topleft_offset = (m_upscale_multiplier != 1 && D3D::IsNvidia(adapter) && !disable_safe_features) ? -0.01f : 0.0f;
	}
